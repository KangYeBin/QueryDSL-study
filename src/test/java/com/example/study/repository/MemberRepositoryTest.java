...
//        teamRepository.save(teamB);
    ...
    void testQueryDSL() {
        ...
        // 클래스 내에 상수로 객체가 선언되어있기 때문에 직접 생성할 필요 없음
        QMember m = member;
...
//                .where(QMember.member.userName.eq(searchName),
//                        QMember.member.age.eq(searchAge))
...
//                .where(원하는 조건)
    ...
    void join() {
        ...
        // Oracle DB의 경우 Oracle의 조인 문법도 사용 가능
    ...
    /*
        ex) 회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조회, 회원은 모두 조회되어야한다
        SQL : SELECT m.*, t.*
                FROM tbl_member m
    ...
    */
    ...
    void subQueryTest() {
        ...
        List<Member> result = factory.selectFrom(member)
                .where(member.age.eq(
                        JPAExpressions  // 서브쿼리를 사용하게 해주는 클래스
    ...
    @Test
    @DisplayName("sub query 사용하기 (나이가 평균 나이 이상인 회원을 조회)")
    void subQueryGoe() {
        // given
        ...
        List<Member> result = factory.selectFrom(member)
                .where(member.age.goe(
                        JPAExpressions
                                // JPQExpression은 from절을 제외하고 select와 where절에서 사용 가능
                                // JPQL도 마찬가지로 from절 서브쿼리 사용 불가
                                // -> Native SQL을 작성하던지, MyBatis or JdbcTemplate 이용
                                .select(memberSub2.age.avg())
                                .from(memberSub2)
                ))
                .fetch();
